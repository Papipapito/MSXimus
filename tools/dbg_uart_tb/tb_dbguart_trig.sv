...
//   3. Sin trig no se emite NADA (que el temporizador no se cuele).
// ============================================================================
`timescale 1ns/1ps
module tb_dbguart_trig;
    ...
    dbg_uart #(.CLK_HZ(53_996_000), .BAUD(115_200), .PERIOD_MS(1), .TRIG_MODE(1)) dut (
    ...
    integer lineas = 0, errores = 0, n = 0, b;
    ...
    initial begin
        ...
        $display("== 1. sin trig no se emite nada ==");
        #2_000_000;                              // 2 ms: con PERIOD_MS=1 el
        if (lineas != 0) begin                   //   temporizador habria disparado
            $display("  FALLO: han salido %0d lineas sin ningun trig", lineas);
            errores = errores + 1;
        end else $display("   silencio, correcto");

        $display("== 2. un trig = una linea ==");
        pulso;
        ...
        $display("== 3. un trig DURANTE la transmision no se pierde ==");
