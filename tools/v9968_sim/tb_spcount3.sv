...
// escritura continua de VRAM por la CPU (la partida dibujando): recorre la
// tabla de nombres y la de color, que es lo que desaloja lineas de la cache
task machaca_vram(input integer nbytes);
integer k;
begin
    vram_set_wr(18'h01800);
    for (k = 0; k < nbytes; k = k + 1) z80_out(2'd0, k[7:0]);
end
endtask
...
initial begin
    ...
    vram_set_wr(18'h03800);        // 3 grupos de patrones solidos
    for (i = 0; i < 96; i = i + 1) z80_out(2'd0, 8'hFF);

    vram_set_wr(18'h01B00);        // 3 sprites en 3 lineas distintas
    ...
    join
    $display("### fin: si la FASE B cae muy por debajo de la A, el bug de DQ2 esta reproducido ###");
    $finish;
end
