...
// ---------------- bus del VDP ----------------
// Protocolo REAL del interface (leccion tb_cpu_bulk): ioreq arma ready;
// valid = PULSO de 1 ciclo. Negedge = sin carreras de scheduler.
...
begin
    ...
    @(negedge clk86);
    while (!bus_ready) @(negedge clk86);
    bus_valid = 1;
    @(negedge clk86);
    bus_valid = 0; bus_ioreq = 0; bus_write = 0;
    repeat (2) @(negedge clk86);
end
endtask
...
begin
    ...
    while (!bus_rdata_en) @(negedge clk86);
    d = bus_rdata;
    repeat (2) @(negedge clk86);
end
endtask
...
begin
    ok = 0;
    for (tt = 0; tt < max_us; tt = tt + 1) begin
        vdp_reg(6'd15, 8'h02);
        bus_rd(3'd1, st);
        vdp_reg(6'd15, 8'h00);
        if (!st[0]) begin ok = 1; tt = max_us; end
        else repeat (86) @(posedge clk86);
    end
    if (!ok) begin
        ce_fails = ce_fails + 1;
        $display("CE COLGADO test %0d", tno);
    end
end
endtask
...
initial begin
    ...
    vdp_reg(6'd20, 8'h01);
    vdp_reg(6'd7,  8'h0F);
    $display("SETUP SC8 OK vs=%0d", vs_count);

    // rectangulos HMMV como sc8hmmv.asm (4 direcciones + grande + impar)
    ...
    $display("HMMV x6 OK (vs=%0d t=%0t)", vs_count, $time);

    // drenar colas y verificar la VRAM FISICA en el modelo W9825
    repeat (40000) @(posedge clk86);
    check_rect(0,  0,  16, 16, 8'd123);
    check_rect(16, 0,  16, 16, 8'd234);   // DIX=1 desde x=31: cubre 16..31
    check_rect(32, 0,  16, 16, 8'd210);   // DIY=1 desde (32,15): 32..47,0..15
    check_rect2();
    scan_mem();
    $display("RESULTADO: ce_colgados=%0d fallos_vram=%0d", ce_fails, vfails);
    if (ce_fails == 0 && vfails == 0) $display("*** SC8 CMD FULL: OK ***");
    else                              $display("*** SC8 CMD FULL: FALLO ***");
    wait (dump_state == 2);
    #1000;
    $display("*** FIN (vs=%0d, bg_miss=%0d) ***", vs_count, shim_diag);
    $finish;
end

// verificacion fisica: pixel SC8 (x,y) -> logico y*256+x -> interleave
// {a17,a0,a16:1} -> byte del W9825 (VRAM_BASE 0x280000, bus 16 bits)
integer vfails = 0;
function [21:0] phys_addr(input [17:0] la);
    phys_addr = 22'h280000 + {4'd0, la[17], la[0], la[16:1]};
endfunction
...
task check_px(input integer x, input integer y, input [7:0] esperado);
    ...
    reg [7:0] leido;
begin
    pa = phys_addr(y*256 + x);
    leido = pa[0] ? sdram.mem[mem_idx(pa)][15:8] : sdram.mem[mem_idx(pa)][7:0];
    if (leido !== esperado) begin
        vfails = vfails + 1;
        if (vfails <= 25)
            $display("VFALLO px(%0d,%0d) leido=%02x esperado=%02x", x, y, leido, esperado);
    end
end
endtask
// red de seguridad: donde aterrizan REALMENTE las escrituras (las unicas
// escrituras wv2 de este guion son los HMMV; el modelo arranca a 0)
task scan_mem;
    integer wi, nz;
begin
    nz = 0;
    for (wi = 0; wi < (1<<24); wi = wi + 1)
        if (sdram.mem[wi] != 16'h0000) begin
            nz = nz + 1;
            if (nz <= 12)
                $display("NOCERO idx=%06x val=%04x (ba=%0d row=%04x col=%03x)",
                         wi, sdram.mem[wi], wi[23:22], wi[21:9], wi[8:0]);
        end
    $display("SCAN: %0d palabras no-cero", nz);
end
endtask
task check_rect(input integer x0, input integer y0, input integer w,
                input integer h, input [7:0] clr);
    integer xx, yy;
begin
    for (yy = y0; yy < y0+h; yy = yy + 1)
        for (xx = x0; xx < x0+w; xx = xx + 1)
            check_px(xx, yy, clr);
end
endtask
task check_rect2;
    integer xx, yy;
begin
    // grande (80,40) 100x60 clr=200 y el impar (101,110) 33x20 clr=77
    for (yy = 40; yy < 100; yy = yy + 1)
        for (xx = 80; xx < 180; xx = xx + 1)
            if (yy < 110 || yy >= 130 || xx < 101 || xx >= 134)
                check_px(xx, yy, 8'd200);
    for (yy = 110; yy < 130; yy = yy + 1)
        for (xx = 101; xx < 134; xx = xx + 1)
            check_px(xx, yy, 8'd77);
end
endtask
